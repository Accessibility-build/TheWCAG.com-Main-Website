    ...
    implementationChecklist: [
      {
        ...
        items: [
          ...
          "Verify elements are nested correctly",
      ...
      },
      {
        ...
        items: [
          ...
          "Use assistive technologies to verify parsing",
        ],
      },
    ],
    officialResources: [
      ...
      },
      {
        ...
        url: "https://www.w3.org/WAI/WCAG22/Techniques/failures/F70",
        type: "Techniques",
      },
    ],
  },
...
function CustomButton({ children, onClick, ariaLabel }) {
  return (
    ...
    keyTerms: [
      {
        term: "Name",
        definition: "The accessible name of a user interface component, which is the text by which the component can be programmatically referred to.",
        context: "The name must be programmatically determinable so assistive technologies can identify and announce the component.",
      },
      {
        term: "Role",
        definition: "The type of user interface component (e.g., button, link, checkbox, textbox).",
        context: "The role must be programmatically determinable so assistive technologies know how to interact with the component.",
      },
      {
        term: "State",
        definition: "The current condition of a user interface component (e.g., checked, expanded, disabled, selected).",
        context: "Component states must be programmatically determinable and communicated when they change.",
      },
      {
        term: "Value",
        definition: "The current value or content of a user interface component (e.g., the text in a textbox, the selected option in a dropdown).",
        context: "Component values must be programmatically determinable so users can understand the current state of inputs and controls.",
      },
    ],
    relatedCriteria: [
      {
    ...
    implementationChecklist: [
      {
        ...
        items: [
          ...
          "Provide accessible names using labels or aria-label",
          "Use appropriate ARIA roles for custom components",
          "Ensure component states are communicated",
          "Make component values programmatically determinable",
        ],
      },
      {
        category: "JavaScript",
      ...
      },
      {
        category: "General",
        items: [
          "Test all components with assistive technologies",
          "Verify name, role, value are announced correctly",
          "Check that state changes are communicated",
    ...
    officialResources: [
      {
        title: "Understanding Name, Role, Value",
...
function StatusMessage({ message, type = 'success' }) {
  return (
    ...
    detailedSummary:
      "What This Means: This success criterion requires that status messages (like form submission success messages, error notifications, loading indicators, or other important updates) are announced to screen reader users without requiring keyboard focus to move to the message. Status messages must be programmatically determinable through ARIA roles (status or alert) or properties.\n\nWhy It's Important: Screen reader users need to be informed about important status updates, but moving focus to status messages can disrupt their workflow. For example, if a user submits a form, they need to know if it was successful, but they shouldn't lose their place on the page. Status messages allow users to be informed of updates without interrupting their current task.\n\nIn Practice: Use ARIA live regions (role='status' or role='alert' with aria-live attributes) to announce status messages. Use role='status' with aria-live='polite' for non-urgent messages like form submission success. Use role='alert' with aria-live='assertive' for urgent messages like errors that require immediate attention. Ensure status messages are in the DOM when they occur so assistive technologies can detect and announce them.",
    keyTerms: [
      {
        term: "Status Message",
        definition: "Information provided to the user about the success or results of an action, the waiting state of an application, the progress of a process, or the existence of errors.",
        context: "Status messages must be programmatically determinable and announced without moving focus.",
      },
      {
        term: "ARIA Live Region",
        definition: "An area of the page that is set up to be announced by screen readers when its content changes, without requiring focus to move to that area.",
        context: "Live regions use role='status' or role='alert' with aria-live attributes to announce updates.",
      },
      {
        term: "Programmatically Determined",
        definition: "Determined by software from author-supplied data provided in a way that different user agents, including assistive technologies, can extract and present this information to users in different modalities.",
        context: "Status messages must be programmatically determinable so assistive technologies can announce them.",
      },
    ],
    relatedCriteria: [
      {
    ...
    implementationChecklist: [
      {
        ...
        items: [
          ...
          "Set aria-live='polite' for most status messages",
          "Set aria-live='assertive' for urgent messages",
          "Use aria-atomic='true' when appropriate",
        ],
      },
      {
        category: "JavaScript",
        items: [
          "Ensure status messages are in the DOM when they occur",
          "Update live regions when status changes",
      ...
      },
      {
        category: "General",
        items: [
          "Test status messages with screen readers",
          "Verify messages are announced without focus moving",
          "Ensure appropriate urgency levels",
          "Test that messages remain in DOM long enough",
        ],
      },
    ],
    officialResources: [
