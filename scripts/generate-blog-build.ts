...
async function generateBlogAtBuild() {
  console.log('📝 Starting build-time blog generation...\n')
  ...
  if (missingVars.length > 0) {
    console.warn('⚠️  Missing environment variables for blog generation:')
    missingVars.forEach((varName) => console.warn(`   - ${varName}`))
    console.warn('\n⚠️  Skipping blog generation at build time.')
  ...
  try {
    ...
    console.log('📰 Extracting articles from last 48 hours...')
    const articles = await extractArticles()
    console.log(`   Found ${articles.length} accessibility articles\n`)

    if (articles.length === 0) {
      console.log('ℹ️  No articles found in the last 48 hours.')
    ...
    // Step 3: Fact-check
    console.log('🔍 Fact-checking blog post...')
    const factCheckResult = await factCheckBlogPost(finalContent, title)
    console.log(`   ${factCheckResult.verified ? '✅ Verified' : '⚠️  Needs Review'}\n`)
    ...
    console.log('✅ Build-time blog generation completed successfully!')
    console.log(`   Blog post: ${slug}`)
    console.log(`   Status: ${isPublished ? 'Published' : 'Needs Review'}\n`)
  } catch (error) {
    console.error('❌ Build-time blog generation failed:', error)
    console.error('\n⚠️  This is not a critical error - the build will continue.')
