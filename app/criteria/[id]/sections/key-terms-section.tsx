...
export function KeyTermsSection({ keyTerms }: KeyTermsSectionProps) {
  ...
  return (
    <section className="mb-12" aria-labelledby="key-terms-heading">
      <div className="flex items-center gap-3 mb-6">
        <BookOpen className="w-6 h-6 text-primary" aria-hidden="true" />
        <h2 id="key-terms-heading" className="text-3xl font-bold">
          Key Terms & Concepts
        </h2>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        {keyTerms.map((term, index) => {
          ...
          return (
            <Card
              key={index}
              className="p-5 border-2 border-primary/10 hover:border-primary/30 transition-colors"
            >
              <button
              ...
              >
                ...
                {isExpanded ? (
                  <ChevronUp className="w-5 h-5 text-primary shrink-0 mt-1" aria-hidden="true" />
                ) : (
                  <ChevronDown className="w-5 h-5 text-primary shrink-0 mt-1" aria-hidden="true" />
                )}
              </button>
              {isExpanded && (
                <div id={`term-content-${index}`} className="mt-4 pt-4 border-t border-primary/10">
