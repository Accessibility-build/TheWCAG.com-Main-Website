...
import { colorizeImage } from "@/lib/tools/converters/image"
...
const tool = getToolBySlug("image-colorizer")!
...
export default function ImageColorizerPage() {
  ...
  const handleColorize = async () => {
    ...
    try {
      const blob = await colorizeImage(file)
      setResult(blob)
      setPreview(URL.createObjectURL(blob))
    } catch (e) {
      setError(e instanceof Error ? e.message : "Colorization failed")
    } finally {
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="space-y-6">
                <Card>
                  ...
                  <CardContent>
                    <FileUpload
                      ...
                      onError={setError}
                      label="Upload Black & White Image"
                      description="Drag and drop a black and white image, or click to browse"
                    />
                  </CardContent>
                </Card>
                ...
                {file && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Colorize Image</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <p className="text-sm text-muted-foreground">
                        Add realistic colors to your black and white photo using AI-powered colorization.
                      </p>
                      <Button
                        ...
                        className="w-full"
                      >
                        {isProcessing ? "Colorizing..." : "Colorize Image"}
                      </Button>
                    </CardContent>
                  </Card>
                )}
              ...
              <div>
                <ConversionResult
                  ...
                  downloadFilename={file ? `${file.name.split(".")[0]}-colorized.png` : undefined}
