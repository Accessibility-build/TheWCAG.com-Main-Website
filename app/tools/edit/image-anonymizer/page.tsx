...
import { anonymizeImage, AnonymizeOptions } from "@/lib/tools/converters/image"
...
const tool = getToolBySlug("image-anonymizer")!

export default function ImageAnonymizerPage() {
  const [file, setFile] = useState<File | null>(null)
  const [method, setMethod] = useState<"gaussian" | "pixelate" | "blackbar">("gaussian")
  ...
  const handleAnonymize = async () => {
    ...
    try {
      ...
      const blob = await anonymizeImage(file, options)
      setResult(blob)
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="space-y-6">
                ...
                {file && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Anonymization Options</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <Button
                            variant={method === "gaussian" ? "default" : "outline"}
                            size="sm"
                          ...
                          <Button
                            variant={method === "pixelate" ? "default" : "outline"}
                            size="sm"
                          ...
                          <Button
                            variant={method === "blackbar" ? "default" : "outline"}
                            size="sm"
                            onClick={() => setMethod("blackbar")}
                          >
                            Black Bar
                          </Button>
                        </div>
                      ...
                      <Button
                        ...
                        className="w-full"
                      >
                        {isProcessing ? "Anonymizing..." : "Anonymize Image"}
                      </Button>
                    </CardContent>
                  </Card>
                )}
              ...
              <div>
                <ConversionResult
                  title="Anonymized Image"
                  downloadData={result || undefined}
                  downloadFilename={file ? `${file.name.split(".")[0]}-anonymized.png` : undefined}
